...
**New copy that states a clinical process (Clarity section, closing paragraph), owner-authored.** Revised 2026-08-26 at owner direction. Current text: "An evaluation gives us the clarity to know what your child needs next. If there is a clear delay, we can begin therapy right away because we are private pay. Families also have the flexibility to begin therapy for mild or emerging concerns before those differences become more significant."
- Both named paths now lead to therapy being available: immediately where there is a clear delay, and optionally where concerns are mild or emerging. This matches the recommendation set documented on evaluations.html §5.
- The claim is about access and scheduling, not clinical outcome. It makes no assertion about prognosis, timelines, or the effect of early intervention, so no citation is required. Do not extend it into an outcome claim (for example "starting earlier leads to better results") without sourcing that separately.
- "Because we are private pay" is a business-model statement: not billing insurance means access is not gated by payer authorization or medical-necessity thresholds. It is accurate as written and should not be restated as a clinical superiority claim.
- Deliberately avoids "wait and see." Owner direction: the practice does not advertise a wait-and-see approach, because families arrive having already been told to wait. This revision strengthens that stance, since the previous version routed mild findings to home strategies with a later check-in, which read closer to waiting. Private pay means direct services remain available for mild or borderline delays. Do not reintroduce wait-and-see phrasing anywhere on the site.
- Superseded wording (do not restore): "If there is a clear delay, we can begin therapy right away. If skills are only mildly behind, we may give you specific strategies to use at home and a plan to check back on their progress."
